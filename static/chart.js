...
const drawChart = async () => {
    ...
    try {
        ...
        if (response.status >= 200 && response.status <= 299){
            ...
            let maxY = formattedData.reduce((acc, cur) => { 
            ...
            let maxDate = formattedData[0][1][0];
            let minDate = formattedData[0][1][formattedData[0][1].length - 1];
            let dateScale = d3.scaleTime().domain([minDate, maxDate]).range([0, w]);
            ...
            let lines = formattedData.forEach((elt, index) => {
